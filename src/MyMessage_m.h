 ...
 *     int msgID; //sequence num
 *     double sendingTime;
 *     int M_Type; //0->Data // 1->ACK // 2->NACK
 *     string M_Payload;
 *     bits crc;
 *     int piggyBackingID; //ACK no.
...
class MyMessage_Base : public ::omnetpp::cPacket
{
  ...
  public:
    ...
    virtual bits& getCrc();
    virtual const bits& getCrc() const {return const_cast<MyMessage_Base*>(this)->getCrc();}
    virtual void setCrc(const bits& crc);
    virtual int getPiggyBackingID() const;
    virtual void setPiggyBackingID(int piggyBackingID);
    virtual int getNumberOfTransmissions() const;
    virtual void setNumberOfTransmissions(int numberOfTransmissions);
};
